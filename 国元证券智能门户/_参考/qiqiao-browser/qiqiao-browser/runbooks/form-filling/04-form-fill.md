...
已从列表视图点击"添加"按钮，打开了表单填写对话框。你可以获取字段列表、填写数据、处理子表，也可以取消返回列表。
...
| 10 | 取消 | 用户不想继续填单 | 关闭表单/返回列表 | → `03-list-view.md` |
...
```
bsk evaluate "document.querySelector('.business_drawer .drawer .drawer_content').scrollTo({ top: document.querySelector('.business_drawer .drawer .drawer_content').scrollHeight, behavior: 'smooth' })"
```

> 滚动容器为 `.drawer_content`，通过父级 `.business_drawer .drawer` 精确定位。
...
这是后续所有操作的基础，**必须先完成**。拿不到 elementData 说明表单未正确打开，退回 `03-list-view.md`。
...
| 流程类型 | 下一节点 |
|----------|---------|
| 填单流 | → `05-popup-handle.md`（弹窗确认）→ `06-site-memory.md`（站点记忆） |
| 发起流程 | → 节点 81（选人/意见弹窗） |

> 发起流程场景的提交流程为：`bsk click @eN 提交` → 节点 81 选人弹窗（搜索+选人+确定）→ 办理弹窗（再确定）→ 完成。

### 操作 10：取消

关闭表单弹窗返回列表视图。先看有没有「取 消」按钮，没有则用 `i.iconclose`。
...
| 失败场景 | 处理方式 | 回退 |
|----------|---------|------|
| `findDom({action:'filling'})` 返回空 | 表单未正确打开，检查页面状态 | → `03-list-view.md` |
...
操作 9 成功 → **`05-popup-handle.md`（弹窗处理 — 提交确认）→ `06-site-memory.md`（站点记忆）**
操作 10 → **`03-list-view.md`（列表视图）**
